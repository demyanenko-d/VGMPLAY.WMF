...
| `0xD0`       | 4    | `pp aa dd`| YMF278B (OPL4) write, порт `pp`              |

Прочие команды пропускаются по таблице длин VGM spec (размер 1, 2, или 3 байта).

### `0xD0` — YMF278B (OPL4) write

```
0xD0 pp aa dd
```

- `pp` бит 7 — выбор чипа (dual-chip); игнорируется, на ZXM-MoonSound один чип.
- `pp & 0x7F` — порт:
  - `0` — FM Bank 0 (те же регистры/порты #C4-#C5, что и `0x5E` для OPL3)
  - `1` — FM Bank 1 (те же регистры/порты #C6-#C7, что и `0x5F` для OPL3)
  - `2` — wave-часть чипа (табличный синтез), порты #7E (адрес) / #7F (данные)
- `aa` — номер регистра, `dd` — значение.

Чтобы чип реально принимал запись в порт 2 (wave-часть), в Bank 1
регистре `0x05` должен быть установлен бит NEW2 (бит 1), в дополнение к
обычному биту NEW (бит 0) — см. `CMDBLK_INIT_OPL4` в `cmdblocks.asm`.
Без NEW2 запись в порты #7E/#7F реальным чипом YMF278B игнорируется.
