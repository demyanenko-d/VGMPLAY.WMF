...
namespace ZXMAK2.Engine.Cpu.Processor
{
    public partial class Z80Cpu
    {
        ...
        {
            (regs._AF, regs.AF) = (regs.AF, regs._AF);
        }

        private void EXX(byte cmd)          // EXX 
        {
            (regs._BC, regs.BC) = (regs.BC, regs._BC);
            (regs._DE, regs.DE) = (regs.DE, regs._DE);
            (regs._HL, regs.HL) = (regs.HL, regs._HL);
        }
        ...
        private static readonly byte[] s_conds =
        [
            CpuFlags.Z, 
            CpuFlags.C, 
            CpuFlags.P, 
