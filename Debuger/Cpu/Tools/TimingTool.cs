...
{
    public class TimingTool(Z80Cpu cpu, Func<ushort, byte> reader)
    {
        private readonly Z80Cpu _cpu = cpu;
        private readonly Func<ushort, byte> _memRead = reader;
        ...
        public int? GetTiming(int addr)
        {
            ...
            while (true)
            {
                ...
                if (perfTime > 1500000)
                {
                    System.Console.Error.WriteLine("TimingTool.GetTiming: {0}T reached!", perfTime); 
                    return null;
                }
            }
            switch (opCode)
