...
{
    public class DasmTool(Func<ushort, byte> memRead, bool isHex = true)
    {
        private readonly Func<ushort, byte> _memRead = memRead;

        public bool IsHex { get; set; } = isHex;
        ...
        public string GetMnemonic(int addr, out int length)
        {
            ...
            do
            {
                if (mnem.Contains("$R", StringComparison.CurrentCulture))  // Prefix register
                {
                ...
                if (mnem.Contains("$PLUS", StringComparison.CurrentCulture))  // PrefixReg+-offset
                {
                ...
                if (mnem.Contains("$S", StringComparison.CurrentCulture))  // Internal bits value
                {
                    var data = _memRead((ushort)(addr + curPtr));
                ...
                if (mnem.Contains("$W", StringComparison.CurrentCulture))  // 2byte value
                {
                ...
                if (mnem.Contains("$N", StringComparison.CurrentCulture))  // 1byte value
                {
                    var data = _memRead((ushort)(addr + curPtr + 1));
                ...
                if (mnem.Contains("$T", StringComparison.CurrentCulture))  // Internal bits value ($S)*8
                {
                    var data = _memRead((ushort)(addr + curPtr));
                ...
                if (mnem.Contains("$DIS", StringComparison.CurrentCulture))  // 1byte offset value
                {
            ...
            } while (mnem.Contains('$', StringComparison.CurrentCulture));
            return mnem;
        }
        ...
        private static readonly string[] DirectZ80Code =
        [
            "NOP",                  // 0x00
			"LD     BC,$W",
			"LD     (BC),A",
        ...
        private static readonly string[] CBhZ80Code =
        [
            "RLC    B",
            "RLC    C",
            "RLC    D",
        ...
        private static readonly string[] EDhZ80Code =
        [
        "",             // 0x00
        "",
        "",
        ...
        private static readonly string[] DDFDhZ80Code =
        [
        "",                     // 0x00
        "",
        "",
        ...
        private static readonly string[] DDFDCBhZ80Code =
        [
        "*RLC   B,($R$PLUS)",      // 0x00
        "*RLC   C,($R$PLUS)",
        "*RLC   D,($R$PLUS)",
