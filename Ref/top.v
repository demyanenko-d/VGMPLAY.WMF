module zx_multisound(
    ...
    output reg fm2_ena,
    output n_saa_cs,
    output n_saa2_cs, // turbo saa
    output saa_clk,
    output saa2_clk,  // turbo saa
    output midi_clk,
...
/* SAA1099 — dual chip via bit7 of address write */
wire port_ff = a[7:0] == 8'hFF && saa_ena && !rom_m1_access;
wire saa_a0 = a[8];

/* Chip select: during address write (saa_a0=1), d[7] selects chip directly;
 * during data write  (saa_a0=0), latched saa_chip is used.
 * SAA1099 ignores upper address bits, so bit7 passes harmlessly. */
reg saa_chip;
reg saa_reg_is_1c;
wire saa_chip_eff = saa_a0 ? d[7] : saa_chip;

assign n_saa_cs  = ~(port_ff && ioreq_wr && saa_chip_eff == 1'b0);
assign n_saa2_cs = ~(port_ff && ioreq_wr && saa_chip_eff == 1'b1);
...
always @(posedge clk32 or negedge rst_n) begin
    ...
    else if (port_fffd_saa && ioreq_wr && d[7:4] == 4'b1111) begin
        saa_clk_en <= ~d[3];
        if (d[3]) saa2_clk_en <= 1'b0;
    end
    else if (port_ff && ioreq_wr && saa_a0) begin
        /* Address write: latch chip select & register id */
        saa_chip      <= d[7];
        saa_reg_is_1c <= (d[6:0] == 7'h1C);
    end
    else if (port_ff && ioreq_wr && !saa_a0 && saa_chip && saa_reg_is_1c && d[0]) begin
        /* Data write to chip 2 reg 0x1C with d[0]=1 (Sound Enable):
         * enable saa2 clock. d[0]=0 (e.g. 0x02 reset) does NOT enable.
         * Disable only via FFFD d[3]=1. */
        saa2_clk_en <= 1'b1;
    end
end

assign saa_clk  = saa_clk_en  ? clk8 : 1'b0;
assign saa2_clk = saa2_clk_en ? clk8 : 1'b0;
