 ...
 * ═══════════════════════════════════════════════════════════════════
 *
 * Частота ISR задаётся вариантом сборки (ISR_FREQ в variant_cfg.h).
 * N_ENTRIES прерываний × STEP T-states = 71680 T = 1 TV-кадр (замыкание).
 *
 * Позиции INT — предрассчитанная таблица POS_TABLE (N_ENTRIES x 5 байт).
...
/** Счётчик тиков ISR. Инкрементируется каждый ISR-вызов (частота = ISR_FREQ),
 *  оборачивается при 256 (полный оборот ≈ 256/ISR_FREQ секунд).
